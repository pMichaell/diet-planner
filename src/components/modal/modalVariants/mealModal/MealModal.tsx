...
const MealModal = ({ idMeal }: { idMeal: string }) => {
  ...
  return (
    <div className={clsx("fillParent", classes.container)}>
      <h2 className={clsx("txtAlgCenter", "centerContents", classes.heading)}>
        {mealDetails?.strMeal}
      </h2>
      <SpinnerImg
      ...
      <section className={clsx("txtAlgCenter", classes.tagsSection)}>
        ...
        <div>
          ...
          <p className={clsx("fw500", "fs600")}>{mealDetails?.strCategory}</p>
        </div>
      </section>
      <button
